...
test('stamp a star -> morph it -> export -> exported path d animates', async ({ page }) => {
  ...
  // Stamp a star: the Star tool draws a center-out drag (center -> radius point).
  // addVectorPath creates a `path` object, selects it, and switches to the node tool.
  ...
  await page.mouse.move(cx, cy - 70); // 70px radius
  await page.mouse.up();
