...
test('Escape cancels the rename (the unmount blur must not re-commit)', async ({ page }) => {
  await page.addInitScript(() => {
    delete (window as unknown as { showSaveFilePicker?: unknown }).showSaveFilePicker;
    delete (window as unknown as { showOpenFilePicker?: unknown }).showOpenFilePicker;
  });
  ...
  await row.locator('span').first().dblclick();
  const input = page.getByTestId(`rename-${rowId}`);
  await input.fill('Nope'); // a different name; Escape must discard it on the unmount blur too
  await input.press('Escape');

  await expect(row).toContainText(original);
  await expect(row).not.toContainText('Nope');
});
