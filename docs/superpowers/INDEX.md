...
polish, led by "author inside a symbol" — **phase 1 (in-symbol DELETE + cross-scene asset prune) is
done**; in-symbol DRAW is next.

**Recommended next (M4) — priority order:**

| Priority | Candidate | Why / source |
|----------|-----------|--------------|
| **1 (next slice)** | **author-in-symbol PHASE 2 — in-symbol DRAW** | ~~delete~~ (phase 1, DONE). Route the geometry-CREATE actions to the active scene + UN-GATE the draw tools in edit mode: `addVectorShape` (rect/ellipse), `addPrimitive` (polygon/star), `addVectorPath` (line/pen/brush). Each appends a new object to the active scene via `commitActiveScene` AND adds the new asset globally to `project.assets`; the 47-edit `setActiveTool` guard (forces `select` in edit mode) must be relaxed to allow the create tools. After this, node-edit + group/boolean + clipboard (cut/copy/paste — note `cut` already deletes inside a symbol but copy no-ops) inside a symbol are the remaining phases. slice47-edit deferrals · author-in-symbol-delete spec §7. |
| 1b (47d polish, small) | symbol library follow-ups: rendered **thumbnails** (a content render-snapshot per symbol), **drag-to-place** with a drop point (v1 is click-to-place), recompute the instance anchor on swap, rename/delete-symbol management in the library | slice47d spec §7 |
