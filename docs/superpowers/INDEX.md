...
| **M4** | Grouping, layers & nested symbols/clips | 🚧 IN PROGRESS — multi-object toolkit (36–44) + grouping phase 1 (42) done; **grouping COMPLETE (45a–45f)**; **boolean ops COMPLETE (46)**; **nested symbols FOUNDATION COMPLETE (47a)** — `SymbolAsset` + recursive `flattenInstances` walker + create-symbol; **instance TRANSFORM UI COMPLETE (47b)** — `instanceAABB` + bbox/scale/rotate handles + move-snapping + surgical live drag-preview; **symbol EDIT MODE COMPLETE (47-edit)** — double-click an instance to edit-in-place its `SymbolAsset.objects` scene (focused-scene view, breadcrumb, transform/animate internals → all instances update). Remaining nested-symbols work: **47c independent timelines** (next priority), 47d library; then "author inside a symbol" (in-symbol draw/delete/group) + bounded grouping/boolean polish |
...
its **foundation (47a)**, **instance transform UI (47b)**, and **symbol EDIT MODE (47-edit)** are
now COMPLETE. Remaining: 47c independent timelines (next), 47d library; then "author inside a
symbol" (in-symbol draw/delete/group).
...
TRANSFORM UI (47b) + SYMBOL EDIT MODE (47-edit) ARE COMPLETE.** A group is a real container with its
...
breadcrumb/Esc to exit), with undo/persistence free and parity untouched (47-edit). The remaining M4
HEADLINE work is 47c–d; everything else is smaller polish.

**Recommended next (M4) — priority order:**

| Priority | Candidate | Why / source |
|----------|-----------|--------------|
| **1 (finish the headline — next slice)** | **47c — independent per-instance timelines** | start-offset + loop / one-shot (± speed) per instance; `localTime = remap(globalTime, instanceChain)`. The engine seam is ALREADY threaded (`InstanceLeaf.localTime`, currently == global time) — this is where two instances of the same symbol finally show DIFFERENT frames at the same wall-clock moment, the real payoff of nested symbols. Needs per-instance timing fields on the instance object + the remap in `flattenInstances` (the one `localTime = time` line) + a small Inspector timing panel. slice47 spec §7. |
| 2 (nested symbols cont.) | **47d — symbols LIBRARY panel** (list symbol defs, drag-to-instance, instance count, swap-symbol, authoring-time cycle guard #2, place-without-selection); then **"author inside a symbol"** — the 47-edit follow-up: route the deferred in-symbol actions (delete w/ cross-scene asset-prune, draw/pen/primitive/brush, node-edit, group/boolean/clipboard, Layers eye/lock/rename/reorder) to the active scene | slice47 spec §7 · slice47-edit deferrals |
