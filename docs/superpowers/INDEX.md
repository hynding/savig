...
| **M4** | Grouping, layers & nested symbols/clips | 🚧 IN PROGRESS — multi-object toolkit (36–44) + grouping phase 1 (42) done; **grouping COMPLETE (45a–45f)**; **boolean ops COMPLETE (46)**; **nested symbols FOUNDATION COMPLETE (47a)** — `SymbolAsset` + recursive `flattenInstances` walker + create-symbol; **instance TRANSFORM UI COMPLETE (47b)** — `instanceAABB` + bbox/scale/rotate handles + move-snapping + surgical live drag-preview; **symbol EDIT MODE COMPLETE (47-edit)** — double-click an instance to edit-in-place its `SymbolAsset.objects` scene; **independent per-instance TIMELINES COMPLETE (47c)** — `symbolTime` (startOffset/loop/speed) + `remapLocalTime` at the `flattenInstances` seam → two instances diverge in frame. Remaining nested-symbols work: **47d library** (next priority); then "author inside a symbol" (in-symbol draw/delete/group) + bounded grouping/boolean polish |
...
ops (46) are COMPLETE. **Nested symbols** — the last M4 headline — is decomposed into 47a–47d; its
**foundation (47a)**, **instance transform UI (47b)**, **symbol EDIT MODE (47-edit)**, and
**independent per-instance TIMELINES (47c)** are now COMPLETE. Remaining: 47d library (next); then
"author inside a symbol" (in-symbol draw/delete/group).
...
**GROUPING (45a–45f) + BOOLEAN OPS (46) ARE COMPLETE; NESTED-SYMBOLS 47a + 47b + 47-edit + 47c ARE
COMPLETE.** A group is a real container with its
...
same playhead (absent = identity → parity-safe; export-remap free) (47c). The remaining nested-symbols
HEADLINE work is **47d (library)**; everything else is smaller polish.

**Recommended next (M4) — priority order:**

| Priority | Candidate | Why / source |
|----------|-----------|--------------|
| **1 (finish the headline — next slice)** | **47d — symbols LIBRARY panel** | list every `SymbolAsset` (with a thumbnail + live instance count), drag-to-instance onto the Stage (place-without-selection), swap-symbol on a selected instance, and the authoring-time cycle guard #2 (reject placing/swapping to a symbol that already contains the target, transitively — render-time guard exists, this is the create-time one). The last nested-symbols headline; turns symbols from "create-from-selection only" into a reusable, browsable library. slice47 spec §7. |
| 2 (nested symbols cont. — bigger) | **"author inside a symbol"** — the 47-edit follow-up: route the deferred in-symbol authoring actions to the active scene so edit mode is a full editor — delete (needs cross-scene asset-prune: an asset shared by another scene must NOT be pruned), draw/pen/primitive/brush, node-edit, group/boolean/clipboard, Layers mutators (eye/lock/rename/reorder). Plus 47c follow-ups: keyframe the timing fields; `SymbolAsset.duration` manual-override + UI; per-instance loop length on the Timeline ruler | slice47-edit deferrals · slice47c spec §7 |
