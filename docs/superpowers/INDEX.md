...
| **M4** | Grouping, layers & nested symbols/clips | 🚧 IN PROGRESS — layers/lock/visibility/reorder + **multi-select (36)** done |
...
**Recommended next (M4, builds directly on slice 36):**

| Candidate | Why / source |
|-----------|--------------|
| **Multi-object MOVE** (drag + arrow-nudge move the whole selection; snapping uses the primary or is suppressed for >1) | slice36 §4 — the natural completion of multi-select; move-drag infra exists |
| **Marquee / rubber-band selection** (drag on empty Stage selects intersecting objects → `selectObjects`) | slice36 §4 |
| **Multi-object transform** (group bbox resize/rotate/scale handles) | slice36 §4 |
| **Grouping** (parent/child; the SceneObject model already has `parentId`/zOrder headroom) | master §10 (M4 headline); needs multi-select ✅ |
...
| Multi-object copy/paste (clipboard is single-object today) | slice36 §4 |
