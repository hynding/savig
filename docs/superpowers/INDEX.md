...
polish, led by "author inside a symbol".

**Recommended next (M4) — priority order:**

| Priority | Candidate | Why / source |
|----------|-----------|--------------|
| **1 (next slice)** | **"author inside a symbol"** — the 47-edit follow-up that turns edit-mode into a FULL editor | 47-edit deliberately routed only the TRANSFORM actions to the active scene; the rest safely no-op. Route the deferred in-symbol authoring actions to `selectActiveObjects`/`commitActiveScene`: **delete** (the subtle one — needs a cross-scene asset-prune: an asset still used by another scene must NOT be pruned, unlike `removeObject`'s root-only check), draw/pen/primitive/brush, node-edit, group/boolean/clipboard, and the Layers mutators (eye/lock/rename/reorder). Decompose if large (delete + draw first, structural next). slice47-edit deferrals. |
