...
- **View-models are framework-neutral:** a view-model imports only `@savig/editor-state` (state type + selectors), `@savig/engine`, `@savig/interaction`. NO react, NO `apps/**`. `intents` take the vanilla `store` (or its `getState`) — they must NOT import React or the app.
